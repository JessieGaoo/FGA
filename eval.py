...
parser.add_argument('--config', default='./configs/Retrieval_coco_test.yaml')
...
parser.add_argument('--epoch', type=int, default=44)
...
model_list = ['ALBEF', 'ViT-B/16', 'RN101', 'BLIP']
record = dict()
...
def retrieval_eval(record, ref_model, tokenizer, blip_tokenizer, target_transform, data_loader, device, config):
    ...
    for batch_idx, (images, texts_group, images_ids, text_ids_groups) in enumerate(tqdm(data_loader)):
        ...
        s_adv_images = images + uap_noise.expand(images.size()) 
        s_adv_images = torch.clamp(s_adv_images, 0.0, 1.0)
        adv_texts = txt_attacker.get_adv_text(source_model, texts)

        with torch.no_grad():
            t_adv_img_list = []
            for itm in s_adv_images:
...
if args.source_model in ['ALBEF', 'TCL', 'BLIP', 'XVLM']:
    ...
    n_px = record['ViT-B/16']['model'].visual.input_resolution  # 224 
    source_transform = transforms.Compose([
        transforms.Resize(n_px, interpolation=Image.BICUBIC),
        transforms.CenterCrop(n_px),
        transforms.ToTensor(),
