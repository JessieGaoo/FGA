...
class CPGCAttacker():
    ...
    def attack(self, imgs, txts, img2txt, txt2img, target_img_supervision, target_txt_supervision, target_img2txt, device='cpu', max_length=30, scales=None, **kwargs):
        with torch.no_grad():
            bs, num_views, c, h, w = imgs.shape
            imgs_flat = imgs.view(bs * num_views, c, h, w)
            normalized_imgs = self.img_attacker.normalization(imgs_flat)
            imgs_outputs = self.model.inference_image(normalized_imgs)
            image_feats = imgs_outputs['image_feat']  # shape: (bs*num_views, 256)
            image_feats = image_feats.view(bs, num_views, -1)  # (bs, num_views, 256)
            img_supervisions = image_feats.mean(dim=1)         # (bs, 256)

        txt_loss, txt_loss_infoNCE, txt_loss_MSE, uap_embedding = self.txt_attacker.img_guided_attack(
            self.model, 
            txts, 
            txt2img, 
        ...
        img_loss, img_loss_infoNCE, img_loss_MSE, uap_noise = self.img_attacker.txt_guided_attack(
            self.model, 
            imgs,
            img2txt, 
            txt2img,
...
class ImageAttacker():
    def __init__(self, netG, normalization, temperature, z, model, eps, device='cuda:2', lr=2e-4, alpha=0.1):
    ...
    def loss_func(self, adv_imgs_embeds, imgs_embeds, txts_embeds, txt2img,target_txt_embeds, target_img2txt, temperature):
    ...
    def txt_guided_attack(self, model, imgs, img2txt, txt2img, device, scales=None, txt_embeds=None, target_txt_embeds=None, target_img2txt=None):
        model.eval()
        ...
        scaled_imgs = self.get_scaled_imgs(aug_imgs)
        ...
        with torch.enable_grad():
            ...
            for i in range(scales_num):
                loss_item = self.loss_func(adv_imgs_embeds[i * b:i * b + b], imgs_embeds[i * b:i * b + b], txt_embeds, txt2img, target_txt_embeds, target_img2txt, self.temperature)
    ...
    def get_scaled_imgs(self, imgs):
        """
        输入: imgs.shape = (bs, 5, 3, 384, 384)
        输出: rearranged_imgs.shape = (bs * 5, 3, 384, 384)
        排列顺序：先取每张图的第0张，再第1张，...最后第4张
        """
        # bs = imgs.shape[0]
        # imgs = imgs.permute(1, 0, 2, 3, 4)  # 转换为：(5, bs, 3, 384, 384)
        # rearranged_imgs = imgs.contiguous().view(bs * 5, 3, 384, 384)  # 展平为：(bs * 5, 3, 384, 384)
        bs, num_views, c, h, w = imgs.shape  # bs=batch size, num_views=5
        # 验证输入张量的元素总数是否正确
        assert bs * num_views * c * h * w == imgs.numel(), "Input tensor size mismatch"
    
        # 使用 transpose 和 reshape 来实现所需排列
        rearranged_imgs = imgs.transpose(0, 1).reshape(bs * num_views, c, h, w)
        
        return rearranged_imgs
 
filter_words = get_filter_words()


class TextAttacker():
    def __init__(self, ref_net, tokenizer, netG=None, z=None, model_name=None, device='cuda:2',temperature=0.1, alpha=None, min_norm=None, max_norm=None, adv_words=None, lr=2e-4, cls=False,max_length=30, number_perturbation=1, topk=10, threshold_pred_score=0.3, batch_size=32):
    ...
    def img_guided_attack(self, net, texts, txt2img, img_embeds=None, target_img_embeds=None):
        ...
        if self.cls:
            ...
            origin_embeds = origin_output['text_feat'].flatten(1).detach() # (5bs,256)
        img_cond = img_embeds[txt2img]  # (5bs, 256)
        padding_zero = []
        position = []
        for i, text in enumerate(texts):
            important_scores = self.get_important_scores(text, net, origin_embeds[i], self.batch_size, self.max_length)  # text中每一个word的重要度分数
            ...
            if self.model_name in ['ViT-B/16', 'RN101']:
                ...
                words, sub_words, keys = self._tokenize(text)  # keys:每个原始词在 sub_words 中的起始和结束索引

                position.append([])
                padding_zero.append([])
                for j in range(self.num_perturbation):
                    position[i].append(keys[list_of_index[j][0]][0])  # 扰动词在分词列表的起始位置
                    padding_zero[i].append(keys[list_of_index[j][0]][1] - keys[list_of_index[j][0]][0] - 1)# 扰动词在分词列表的结束位置-起始位置
        with torch.enable_grad():
            uap_embedding = self.generator(self.z, img_cond)[0]  # [1,16,48]
            uap_embedding = uap_embedding.reshape(uap_embedding.size(0), -1) # [1,768]
            ...
            loss_infoNCE = self.loss_func(adv_txt_embeds, img_embeds, txt2img, target_img_embeds, self.temperature)  # 多模态对比损失
            criterion_MSE = torch.nn.MSELoss(reduce=True, size_average=False)
            loss_MSE = criterion_MSE(adv_txt_embeds, origin_embeds)  # 单模态对比损失
            loss = loss_infoNCE - self.alpha * loss_MSE
        loss.backward()
        self.optimG.step()
    ...
    def loss_func(self, adv_txt_embeds, img_embeds, txt2img, target_img_embeds, temperature):
        ...
        index = torch.min(similarity, dim=-1).indices  # 与文本最不相似的图像索引
        target_it_labels = torch.zeros_like(target_it_sim_matrix).to(device)
        for i in range(target_it_labels.size(0)):
